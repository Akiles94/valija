...
const ENTRY_SHELL = readFileSync(join(import.meta.dirname, "components/entry-shell.tsx"), "utf8");
 ...
 * still never imports `useTheme`) and the entry cluster's own permanent
 * `dark` brand panel (GUI-LAYOUT D-20, Slice 11's second sanctioned static-
 * dark subtree). A DOM-level render test isn't available here — P-D5
 * confines jsdom to exactly `recovery-kit.tsx` and `relocate-vault.tsx` — so
 * this proves the wiring the same structural way `no-network-surface.test.ts`
 * and `diagnostics.no-auto-run.test.ts` do.
 */
describe("the resolved theme reaches the DOM in exactly one place", () => {
  ...
  it("entry-shell.tsx hardcodes dark and never imports theme-context", () => {
    expect(ENTRY_SHELL).toContain('data-theme="dark"');
    expect(ENTRY_SHELL).not.toMatch(/from ["'].*theme-context\.js["']/);
  });
});
